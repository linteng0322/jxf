...
<body class="home">
	<div id="page">
		...
		<div style="float: right; width: 80%;">
			...
			<sf:form servletRelativeAction="alljxforder" method="post"
					...
					<table
						...
						<c:forEach var="jxforder" items="${page.list}" varStatus="loop">
							...
							<tr>
								...
								<td><c:if test="${jxforder.jxforderstatus=='completed'}">
										<c:out value="出库完成"></c:out>
									</c:if> <c:if test="${jxforder.jxforderstatus=='partial'}">
										<a
											href="<c:url value="/jxforder/matchmaterialforjxforder?id=${jxforder.id}"/>"
											style='color: red'><c:out value="部分出库"></c:out></a>
									</c:if> <c:if test="${jxforder.jxforderstatus=='draft'}">
										<c:out value="草稿"></c:out>
									</c:if> <c:if test="${jxforder.jxforderstatus=='allset'}">
										<a
											href="<c:url value="/jxforder/matchmaterialforjxforder?id=${jxforder.id}"/>"
											style='color: red'><c:out value="需出库"></c:out></a>
									</c:if> <%-- <c:out value="${jxforder.jxforderstatus=='completed'?'出库完成':${jxforder.jxforderstatus=='partial'?'部分出库':'需出库'}" /></td> --%>
