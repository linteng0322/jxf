...
<script type="text/javascript">
	...
	function initleibietables() {
		...
		for (var i = 0; i < totalorderlength; i++) {
			...
			addElement(leibie, identity, materialId, thickness, color, length,
					materialcount, pinming, materialstatus, missingcount);
		}
		var pricetitle = "<table id='pricetable'  class='table table-striped table-bordered table-hover table-condensed' style='font-size:10px' >"
				+ "	<thead><tr><th>类别</th>"
				+ "			<th>理论重量</th>"
				+ "			<th>实际重量</th>"
	...
	function addElement(typenumber, identity, materialId, thickness, color,
			length, materialcount, pinming, materialstatus, missingcount,
			checkboxstatus) {
		...
		var checkboxstring = (materialstatus != "completed") ? originalcheckbox
				: updatedcheckbox;
		if (materialstatus == "set")
			materialstatus = "已匹配";
		if (materialstatus == "completed")
			materialstatus = "出库完成";
		if (materialstatus == "partial")
			materialstatus = "部分出库";
		...
		var missingcountstring = (missingcount != 0) ? originalmisscount
				: updatedmissingcount;
		var pinmingred = "";
		if (pinming == "null") {
		...
		var appendstring = "<tr><td>"
				...
				+ "' value='"
				+ materialId
				+ "' list='json-datalist' style='width: 150px; padding: 4px; border: none; background: transparent;'"
	...
	function check() {
		if (confirm('确实要出库吗?')) {
			...
			for (var j = 1; j <= l; j++) {
				var searchmaterial = $(
						'input:text[id="additionalmaterialcount' + j + '"]')
						.val();
				if (searchmaterial == "") {
		...
		<div style="float: right; width: 100%;">
			...
			<sf:form servletRelativeAction="jxfordertotransactionout"
				...
				<sp:message code="label.order" />:<c:out value='${order.orderId}' />&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
					<sp:message code="label.customer" />:<c:out
					value='${order.customer.name}' />&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
					<sp:message code="label.orderdate" />:<c:out
				...
				<table style="width: 100%;"
					...
					<tr>
						<td>配件 <sf:input path="additionalmaterialstring"
								...
								<thead>
									<tr>
										<th><sp:message
												code="label.id" /></th>
										<th><sp:message
												code="label.additionalmaterial" /></th>
										<th>数量</th>
										<th>单价</th>
									</tr>
								</thead>
								<tbody>
									<c:forEach var="additionalmaterial"
										...
										<tr>
											<td>${loop.index+1}</td>
											<td><c:out
													value="${additionalmaterial[0]}" /></td>
											<td><c:out
													value="${additionalmaterial[1]}" /></td>
											<td><c:out
													value="${additionalmaterial[2]}" /></td>
										</tr>
									</c:forEach>
							</table> <input id="matchmaterial" name="matchmaterial" type="hidden"
							value="abc">
							<p />
						</td>
						<td>价目<div id="materialpricelist"></div></td>
					</tr>
				</table>
				<br />
				...
				<sp:message code="label.actweight" />:<c:out
					value='${order.actweight}' />&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
				汇总金额:<c:out value='${order.totalincome}' />
				<p />
				<sp:message code="label.salesman" />:<c:out
					value='${order.salesman.name}' />&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
