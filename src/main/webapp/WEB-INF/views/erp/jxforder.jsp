	...
	function submitorder() {
		...
		for (var j = 1; j <= i; j++) {
			...
			if (additionalmaterialname != "") {
				if (flag == "") {
					additionalmaterialstring = additionalmaterialname+";"+additionalmaterialcount;
					flag = "has";
				} else {
					additionalmaterialstring = additionalmaterialstring + ","
							+ additionalmaterialname+";"+additionalmaterialcount;
				}
			}
		}
							...
							<c:forEach var="ordermaterial" items="${order.materialorderlist}"
								...
								<tr>
									...
									<td><input list="json-datalist"
										...
										style='width: 100px; padding: 4px; text-transform: uppercase' oninput="inputthisrow(<c:out value="${loop.index+1}"/>, this)"/>
										<datalist id="json-datalist">
										</datalist></td>
