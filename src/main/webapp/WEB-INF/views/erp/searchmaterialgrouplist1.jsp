...
<script type="text/javascript">
	function selectMaterialGroup() {
		...
		var val = $('input:radio[name="materialgroupId"]:checked').val();
		var materialIdString = $('input:radio[name="materialgroupId"]:checked').parent().next().text();
		materialIdString = materialIdString.replace("\n", "").replace("\n","").trim();
		var thicknessString = $('input:radio[name="materialgroupId"]:checked').parent().next().next().text();
		thicknessString = thicknessString.replace("\n", "").replace("\n","").trim();
		var colorString = $('input:radio[name="materialgroupId"]:checked').parent().next().next().next().text();
		colorString = colorString.replace("\n", "").replace("\n","").trim();
		var lengthString = $('input:radio[name="materialgroupId"]:checked').parent().next().next().next().next().text();
		...
		var stockString = $('input:radio[name="materialgroupId"]:checked').parent().next().next().next().next().next().text();
...
<body class="home">
	<div id="page">
		...
		<div style="width: 100%;">
			...
			<sf:form servletRelativeAction="searchmaterialgroup" method="post"
				...
				<div class="panel panel-default"
					...
					<table
						...
						<c:forEach var="materialgroup" items="${materialgrouplist}" varStatus="loop">
							<tr>
								<td><sf:radiobutton path="materialgroupId"
										value="${materialgroup.id}" /></td>
